...
Drizzle ORM done right. `pgTable` schema design, `relations()`, `$inferSelect`/`$inferInsert` type inference, prepared statements, `drizzle-kit` migrations, relational queries, `drizzle-zod`, and connection pooling.

> AI coding assistants default to Prisma syntax, write raw SQL instead of using the query builder, manually define types that mirror schemas, and skip `drizzle-kit` for migrations. This plugin anchors your agent to Drizzle's actual API.
...
- [typescript-best-practices](https://github.com/ofershap/typescript-best-practices) - TypeScript 5.x strict patterns for type-safe queries
- [fastapi-best-practices](https://github.com/ofershap/fastapi-best-practices) - FastAPI patterns (if using Drizzle with a Python backend via API)
